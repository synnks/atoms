...
final class MapReduceFunctionOps[K <: HList, IR <: HList, R](val f: MapReduceFunction[K, IR, R]) extends AnyVal {
  ...
  infix def andThen[K2 <: HList, IR2 <: HList, R2](g: MapReduceFunction[K2, IR2, R2])(implicit
    andThen: AndThen[K, IR, R, K2, IR2, R2]
  ): andThen.Out = andThen(f, g)

  infix def compose[K2 <: HList, IR2 <: HList, R2](g: MapReduceFunction[K2, IR2, R2])(implicit
